...
public class FrmSearch extends JDialog {
	...
	private JLabel lableMonth = new JLabel("关键字");
	...
	private void reloadTable(){
		try {
			List<Payment> records=PaymentManager.Search(this.edtKeyword.getText());
	...
	private void reloadPaymentInfo(){
		...
		if(!"".equals(s)){
			try {
				pp=(new PaymentManager()).Search(s);
			} catch (BaseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public FrmSearch(Frame f, String s, boolean b) {
		...
		this.edtKeyword.getDocument().addDocumentListener(new javax.swing.event.DocumentListener(){
		    public void changedUpdate(DocumentEvent e) {//这是更改操作的处理
		    	FrmSearch.this.reloadPaymentInfo();
		  	}
			@Override
